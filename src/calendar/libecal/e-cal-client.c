	...
	if (e_cal_component_has_recurrences (ci->comp) &&
		...
		if (instances_hold->start_zone) {
			itt = i_cal_time_from_timet_with_zone (i_cal_time_as_timet (start),
				dtstart && e_cal_component_datetime_get_value (dtstart) && i_cal_time_is_date (e_cal_component_datetime_get_value (dtstart)),
				instances_hold->start_zone);
			if (dtstart)
				e_cal_component_datetime_set_tzid (dtstart, i_cal_timezone_get_tzid (instances_hold->start_zone));
			...
			itt = i_cal_time_from_timet_with_zone (i_cal_time_as_timet (start), TRUE, instances_hold->default_zone);
		} else {
			itt = i_cal_time_from_timet_with_zone (i_cal_time_as_timet (start),
				dtstart && e_cal_component_datetime_get_value (dtstart) && i_cal_time_is_date (e_cal_component_datetime_get_value (dtstart)),
				NULL);
		...
		if (dtstart)
			...
			dtstart = e_cal_component_datetime_new (itt, instances_hold->start_zone ? i_cal_timezone_get_tzid (instances_hold->start_zone) : NULL);
		...
		if (instances_hold->end_zone) {
			itt = i_cal_time_from_timet_with_zone (i_cal_time_as_timet (end),
				dtend && e_cal_component_datetime_get_value (dtend) && i_cal_time_is_date (e_cal_component_datetime_get_value (dtend)),
				instances_hold->end_zone);
			if (dtend)
				e_cal_component_datetime_set_tzid (dtend, i_cal_timezone_get_tzid (instances_hold->end_zone));
			...
			/* Floating date, set in the default zone */
			itt = i_cal_time_from_timet_with_zone (i_cal_time_as_timet (end), TRUE, instances_hold->default_zone);
		} else {
			itt = i_cal_time_from_timet_with_zone (i_cal_time_as_timet (end),
				dtend && e_cal_component_datetime_get_value (dtend) && i_cal_time_is_date (e_cal_component_datetime_get_value (dtend)),
				NULL);
		...
		if (dtend)
			...
			dtend = e_cal_component_datetime_new (itt, instances_hold->end_zone ? i_cal_timezone_get_tzid (instances_hold->end_zone) : NULL);
                    ...
                    gpointer cb_data)
{
	...
	for (l = objects; l && !g_cancellable_is_cancelled (cancellable); l = l->next) {
		...
		if (e_cal_component_is_instance (comp)) {
			 ...
			 * timezone. */
			if (dtstart && e_cal_component_datetime_get_tzid (dtstart) && e_cal_component_datetime_get_value (dtstart) &&
			    !i_cal_timetype_get_is_date (e_cal_component_datetime_get_value (dtstart))) {
				start_zone = e_cal_client_tzlookup_cb (e_cal_component_datetime_get_tzid (dtstart), client, cancellable, NULL);
				if (!start_zone)
			...
			if (dtend && e_cal_component_datetime_get_tzid (dtend) && e_cal_component_datetime_get_value (dtend) &&
			    !i_cal_timetype_get_is_date (e_cal_component_datetime_get_value (dtend))) {
				end_zone = e_cal_client_tzlookup_cb (e_cal_component_datetime_get_tzid (dtend), client, cancellable, NULL);
				if (!end_zone)
			...
			ci->start = i_cal_time_as_timet_with_zone (e_cal_component_datetime_get_value (dtstart), start_zone);

			if (dtend && e_cal_component_datetime_get_value (dtend))
				ci->end = i_cal_time_as_timet_with_zone (e_cal_component_datetime_get_value (dtend), end_zone);
			...
			if (datetime && e_cal_component_datetime_get_tzid (datetime))
				e_cal_client_get_timezone_sync (client, e_cal_component_datetime_get_tzid (datetime), &start_zone, cancellable, NULL);
			else
				start_zone = NULL;
			...
			if (datetime && e_cal_component_datetime_get_tzid (datetime))
				e_cal_client_get_timezone_sync (client, e_cal_component_datetime_get_tzid (datetime), &end_zone, cancellable, NULL);
			else
				end_zone = NULL;
			e_cal_component_datetime_free (datetime);
	...
	for (l = instances; l && !g_cancellable_is_cancelled (cancellable); l = l->next) {
		...
		result = (* cb) (e_cal_component_get_icalcomponent (ci->comp), starttt, endtt, cb_data, cancellable, NULL);
		...
		if (result) {
			...
			if (rid && *rid) {
				if (instance_rid && *instance_rid && strcmp (rid, instance_rid) == 0) {
					starttt = i_cal_time_from_timet_with_zone (ci->start, FALSE, client->priv->default_zone);
					endtt = i_cal_time_from_timet_with_zone (ci->end, FALSE, client->priv->default_zone);

					result = (* cb) (e_cal_component_get_icalcomponent (ci->comp), starttt, endtt, cb_data, NULL, NULL);
				}
			} else {
				starttt = i_cal_time_from_timet_with_zone (ci->start, FALSE, client->priv->default_zone);
				endtt = i_cal_time_from_timet_with_zone (ci->end, FALSE, client->priv->default_zone);
                                                 ...
                                                 gpointer cb_data)
{
	...
	datetime = e_cal_component_get_dtstart (comp);
	if (datetime && e_cal_component_datetime_get_tzid (datetime))
		e_cal_client_get_timezone_sync (client, e_cal_component_datetime_get_tzid (datetime), &start_zone, NULL, NULL);
	else
		start_zone = NULL;
	e_cal_component_datetime_free (datetime);
	...
	datetime = e_cal_component_get_dtend (comp);
	if (datetime && e_cal_component_datetime_get_tzid (datetime))
		e_cal_client_get_timezone_sync (client, e_cal_component_datetime_get_tzid (datetime), &end_zone, NULL, NULL);
	else
		end_zone = NULL;
	e_cal_component_datetime_free (datetime);
