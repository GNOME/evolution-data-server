...
get_revision_compare_cycle (EBookClient *client)
{
       ...
       if (!e_client_get_backend_property_sync (client, BOOK_BACKEND_PROPERTY_REVISION, &revision_before, NULL, &error))
	...
	g_object_unref (contact);
       ...
       if (!e_client_get_backend_property_sync (client, BOOK_BACKEND_PROPERTY_REVISION, &revision_after, NULL, &error))
               g_error ("Error getting book revision: %s", error->message);
